...
from formulas2 import update, init_learned, \
    init_weight, init_guess, init_slip, init_transit, GuessPMF, SlipPMF
...
def main(num_learners=1000, num_cards=50):
    ...
    my_cards = [{
        ...
        'guess_distro': GuessPMF({
            h: 1 - (init_guess - h) ** 2
            for h in [h / 100 for h in range(1, 70)]}),
        'slip': init_slip,
        'slip_distro': SlipPMF({
            h:  1 - (init_slip - h) ** 2
            for h in [h / 100 for h in range(1, 40)]}),
